    ...
    <div className='bg-black/90 m-auto text-white px-3 py-4 w-screen h-screen fixed top-0 left-0 z-10 overflow-hidden lg:px-10 lg:py-6'>
      <div className='max-w-[1536px] m-auto'>
        <div className='flex items-center justify-between'>
          <h2 className='text-base tracking-wide mr-2 md:text-lg'>{title}</h2>
        ...
        <div className={`items-center overflow-y-scroll mt-4 mb-6 lg:px-24 ${screenSize.height < 400 ? 'h-[300px]' : screenSize.height < 550 ? 'h-[450px]' : screenSize.height < 700 ? 'h-[550px]' : screenSize.height < 750 ? 'h-[660px]' : 'h-[700px]'} ${screenSize.width < 800 && 'no-scrollbar'}`}>
          {images.map((image) => {
            return (
              <div className='w-full my-4 h-[250px] px-2 md:h-[750px]' key={image}>
                <img src={image.startsWith('/') ? `${process.env.NEXT_PUBLIC_API_SRV}${image}` : image} alt={image} className='w-full h-full aspect-square' />
              </div>
            )
          })}
        </div>
